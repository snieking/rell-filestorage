...
sed -i "s/FILEHUB_BRID/${FILEHUB_BRID}/g" run.xml
sed -i "s/FILECHAIN_UID/$(($(date +%s%N)/1000000))$(od -An -N1 -i /dev/random)/g" run.xml
...
./build.sh
echo "Filechain compiled successfully!"

echo "Starting Filechain..."
./run-dev-node.sh